...
public class ConfigSync
{
  ...
  private static void AddValueToZPackage(ZPackage package, object? value)
  {
    ...
    if (Serialize(value, ref package)) return;
  ...
  private static bool Serialize(object obj, ref ZPackage pkg)
  {
    if (obj is int i)
    {
      pkg.Write(i);
  ...
  {
    var ret = Deserialize(type, package);
    if (ret != null)
      return ret;
    ...
    if (type.IsClass || type.IsValueType)
    {
      ...
      foreach (FieldInfo field in fields)
      {
        ...
        {
          Log.Error($"Failed to deserialize field {field.Name} of type {field.FieldType} in struct {type}");
